...
public interface UserDao {
	...
	@Insert("insert into users values (#{uid}, #{pwd}, #{uname}, #{email}, default, default, #{profile}, #{github}, #{insta}, #{location})")
	void insertUser(User user);
	
	@Update("update users set pwd=#{pwd}, uname=#{uname}, email=#{email}, profile=#{profile}, github=#{github}, insta=#{insta}, location=#{location} where uid=#{uid}")
